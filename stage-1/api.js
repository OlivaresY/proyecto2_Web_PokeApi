...
export function getValidMoves(data) { //Función que filtra y selecciona los movimientos correctos del Pokemon
    if (!data || !data.moves || data.moves.length === 0) return []; //Validación: si no hay datos o no hay movimientos → devuelve un arreglo vacío.
    const levelUpMoves = data.moves // Se emepieza con todos los movimientos del Pokemon.
        ...
        .map(move => {
            const details = move.version_group_details.filter(v => v.move_learn_method.name === "level-up");
            const minLevel = Math.min(...details.map(v => v.level_learned_at));
                return { ...move, minLevel };
        });
    levelUpMoves.sort((a, b) => a.minLevel - b.minLevel); //Ordenas los movimientos de menor nivel a mayor nivel.
    return levelUpMoves.length > 5 ? levelUpMoves.slice(-5) : levelUpMoves; //Tomas solo los últimos 5 movimientos (los de nivel más alto).
}
